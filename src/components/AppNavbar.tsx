...
import {
  Menu, X, Home, Wrench, FilePlus, Star, LogOut, ChevronDown,
  UserCircle, Sparkles, Lock, User
} from "lucide-react";
...
const AppNavbar = () => {
  ...
  const { user, isGuest, guestId, signOut, exitGuest } = useAuth();

  const navLinks = [
    { label: "Home", to: "/home", icon: Home, guestLocked: true },
    { label: "Tools", to: "/tools", icon: Wrench, guestLocked: false },
    { label: "Create Doc", to: "/create-doc", icon: FilePlus, guestLocked: true },
    { label: "Favorites", to: "/favorites", icon: Star, guestLocked: true },
  ];
  ...
  const handleGuestNavClick = (link: typeof navLinks[0]) => {
    if (isGuest && link.guestLocked) {
      setUpgradeModal({ open: true, featureName: link.label });
      setOpen(false);
    }
  };

  // Guest display: unique guest ID as profile name
    ...
    <>
      ...
      <nav className="nav-blur fixed top-0 left-0 right-0 z-50 border-b border-border">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          {/* Desktop Nav — show all links for everyone */}
          <div className="hidden md:flex items-center gap-1">
            {navLinks.map((l) => {
              const locked = isGuest && l.guestLocked;
              const active = !locked && (location.pathname === l.to || (l.to !== "/home" && location.pathname.startsWith(l.to)));

              if (locked) {
                return (
                  <button
                    key={l.label}
                    onClick={() => handleGuestNavClick(l)}
                    className="flex items-center gap-1.5 px-4 py-2 rounded-full text-sm font-semibold transition-all duration-200 text-muted-foreground/50 hover:text-muted-foreground hover:bg-muted/30 relative"
                  >
          ...
          {/* Right: guest profile chip or user dropdown */}
          <div className="hidden md:flex items-center gap-3 relative">
            {isGuest ? (
              <div className="flex items-center gap-2">
                {/* Guest profile chip */}
                <button
                  onClick={() => setUpgradeModal({ open: true, featureName: "Profile" })}
                  className="flex items-center gap-2 px-3 py-2 rounded-full border border-dashed border-border hover:border-primary/40 transition-all duration-200 bg-card/50"
                >
                  ...
                  <div className="text-left">
                    <p className="text-xs font-bold leading-tight max-w-[100px] truncate">{guestId}</p>
                    <p className="text-[10px] text-muted-foreground leading-tight">Guest mode</p>
                  </div>
                </button>
        ...
        {isGuest && (
          <div className="border-t border-border px-6 py-2 flex items-center justify-between gap-4"
            ...
            <p className="text-xs text-muted-foreground flex items-center gap-1.5">
              <span className="font-bold font-mono text-[11px] px-1.5 py-0.5 rounded-md border border-dashed border-border bg-background/60">
                {guestId}
              </span>
              <span>· Guest session · Only PDF tools are active.</span>
            </p>
          ...
          <div className="md:hidden bg-card/95 backdrop-blur border-b border-border px-6 py-4 flex flex-col gap-3">
            {navLinks.map((l) => {
              ...
              return locked ? (
                <button key={l.label}
                  onClick={() => handleGuestNavClick(l)}
                  className="flex items-center gap-2 text-sm font-semibold text-muted-foreground/50">
            ...
            <div className="border-t border-border pt-3 mt-1">
              {isGuest ? (
                <div className="space-y-2">
                  <div className="flex items-center gap-2 px-3 py-2 rounded-xl border border-dashed border-border bg-background/50">
                    <User className="w-4 h-4 text-muted-foreground" />
                    <span className="text-xs font-mono font-bold text-muted-foreground">{guestId}</span>
                  </div>
